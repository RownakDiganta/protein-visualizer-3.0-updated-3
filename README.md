<!-- README.md -->
<!-- Project overview: what the visualizer does, its features, and links to prior versions/citations. -->

# Read Me

## Amino-Acid Grouped Label Visualization

This version assigns one consistent vertical label level to each selected amino-acid type. All occurrences of the same amino acid share one primary connector-line length and label height, while different amino-acid types occupy separate vertical levels. Within a single amino-acid type, labels that would otherwise overlap in dense regions are nudged onto a small number of nearby sub-levels so every letter and residue number stays readable - these sub-levels stay tightly grouped within that type's own band and never cross into another amino acid's level. This preserves equal treatment within each amino-acid group while visually separating different residue types.
